...
router.get("/utente/:idUtente", async (req, res) => {
  let chat = await Chat.find({ destinatario: req.params.idUtente });

  if (chat.length == 0) {
    chat = await Chat.find({ mittente: req.params.idUtente });
    if (chat.length == 0) {
  ...
  chat = chat.map((dbEntry) => {
    return {
      self: "/chat/" + dbEntry.id,
      mittente: dbEntry.mittente,
      destinatario: dbEntry.destinatario,
      data: dbEntry.data,
      text_message: dbEntry.text_message,
    };
  });
...
router.post("", async (req, res) => {
  ...
  let utenteMedicoId = utenteMedicoUrl.substring(
    utenteMedicoUrl.lastIndexOf("/") + 1
  );
